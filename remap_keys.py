...
def set_scancode_map(remap_dict):
    ...
    # Each mapping: TO TO FROM FROM
    for from_code, to_code in remap_dict.items():
        data += scancode_dictionary[to_code].to_bytes(2, 'little')
        data += scancode_dictionary[from_code].to_bytes(2, 'little')
